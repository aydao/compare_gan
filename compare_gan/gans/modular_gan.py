...
class ModularGAN(AbstractGAN):
  ...
  def label_generator(self, shape, name=None):
    ...
    if label_filenames is not None and len(label_filenames) > 0:
      ...
      v = tf.Variable(self._dataset._all_labels, name="labels_var", dtype=tf.int32, trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
      logging.info("Returning real random labels %s", v)
